...
用法：
  1. 账号来源（二选一，优先 sf_openid 自动换 ck，失败回退 sfsyUrl）：
     wx_server_url    应用宝网关地址
     sf_openid  微信 openid（多账号用 &/逗号/换行分隔），配合 wx_server_url 应用宝网关
     sfsyUrl    URL 或 Cookie 格式账号（多账号换行分隔）
  2. 可选环境变量：
     SFBF          并发数（默认 1）
     ENABLE_PROXY  是否启用代理（默认 false）
     SF_PROXY_API_URL  代理接口地址
     SF_AUTO_COOKIE   是否启用 sf_openid 自动换 ck（默认 1）
...
from urllib.parse import unquote, quote as url_encode
...
# ==================== AutoCookie (sf_openid 登录态自动换 ck) ====================
# 与 sf.py 对齐：通过微信 OAuth 路径（wx_server_url 网关 + sf_openid）自动换取顺丰 cookie，
# 完全不走 universalSign/deviceId，不会顶掉 App 端 deviceId 会话。
SF_WX_SERVER = (os.environ.get("wx_server_url") or "").strip().rstrip("/")
SF_WX_APPID = "wxd4185d00bf7e08ac"       # 小程序 appid（硬编码）
SF_PUBLIC_ID = "gh_f9d9fca26a50"          # 小程序原始ID（硬编码）
...
def _log_global(msg: str):
    """AutoCookie 流程的全局日志（sfzq.py 主流程用 print，这里对齐 sf.py 的 _log_global）"""
...
class AutoCookieManager:
    """微信 OAuth 路径自动换取顺丰 Cookie（与 sf.py 同逻辑）。"""
    ...
    def get_cookie_for_wxid(self, wxid: str) -> Optional[str]:
        """通过 /wxapp/getCode 拿到 code 后，走 UCMP 换顺丰 Cookie（与 sf.py 一致）。"""
...
def _auto_fetch_cookies() -> List[str]:
    """读取 sf_openid（多账号 &/逗号/换行分隔），经微信 OAuth 路径换取顺丰 cookie 列表。"""
...
def main():
    # 优先走 sf_openid 微信 OAuth 自动换 ck（与 sf.py 一致）；否则回退 sfsyUrl
    account_urls: List[str] = []
    if SF_AUTO_COOKIE and SF_OPENIDS:
        account_urls = _auto_fetch_cookies()

    if not account_urls:
        env_name = 'sfsyUrl'
        env_value = os.getenv(env_name)
        if not env_value:
            print("❌ 未捕获到在线顺丰账号凭证（sf_openid / sfsyUrl 均未配置或换 ck 失败）")
            return
